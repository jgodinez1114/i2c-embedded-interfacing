...
DigitalOut rst(p28);  
DigitalOut data(p30); 
DigitalOut myled(p14); //Give the LED1 a name, such as myled
DigitalOut myled2(p39); //Give the LED1 a name, such as myled
DigitalIn toggle(p23,PullUp); //Declare the user switch via pin 23
// LCD Commands
// -------------------------------------------------------------------------
...
//	}
	...
	//continuously read from VEML7700
	while(1){
		...
		ALS = decode_bcd(ALS);
		white = decode_bcd(white);
		ALS_INT = decode_bcd(ALS_INT);
...
wait (3);
	...
	lcd_command(LCD_CLEARDISPLAY);
		while(1){
				myled=1; //Switch the LED ON
				wait(.2); //Wait for 0.2 seconds
				myled=0;
				wait(.2);
		...
		lcd_data('d');
		lcd_data(' ');
		if(!toggle){ //If the switch is not pressed, keep the LED ON
			//If the switch is pressed, turn OFF the LED
			lcd_command(LCD_CLEARDISPLAY);
			wait(5);
								lcd_data('C');
								lcd_data('u');
